...
def selectBranch():
    ...
    while libInp != (numBranches + 1):
        ...
        for i in range(0, len(libraries)):
            ...
            print("{0}) {1}, {2}".format(
                libraries[i][0], libraries[i][1], libraries[i][2]))
        print("{0}) Quit to previous page\n".format(numBranches + 1))

        # Take input from user and take appropriate action
        libInp = input(
            "Please enter a number between 1 and {0}: ".format(numBranches + 1))
        if check.validInput(libInp, 1, numBranches + 1):
            ...
            if libInp == (numBranches + 1):
                ...
                adjustLibrary(
                    branchIds[libInp-1], libraries[libInp-1][1], libraries[libInp-1][2], libraries)
...
def changeBranchInfo(branchId, branchName, branchLocation, libraries):
    ...
    print("\nYou have chosen to update the Branch with Branch Id:{0} and Branch Name:{1}.".format(
        branchId, branchName))
...
def updateBookCount(branchId, branchName, branchLocation, table):
    ...
    for i in range(0, len(listOfCopies)):
        print("{0}) {1} {2}".format(
            i+1, getBookTitle(listOfCopies[i][0]), listOfCopies[i][2]))
    print("{0}) Quit to previous page\n".format(numBooks + 1))

    # Take input from user and take appropriate action
    bookInp = input(
        "Please enter a number between 1 and {0}: ".format(numBooks + 1))
    if check.validInput(bookInp, 1, numBooks + 1):
        bookInp = int(bookInp)
        if bookInp != (numBooks + 1):
            print("\nYou picked {0} {1}".format(
                listOfCopies[bookInp-1][0], getBookTitle(listOfCopies[bookInp-1][0])))
            print("\nExisting number of copies: {0}".format(
                listOfCopies[bookInp-1][2]))
            ...
            if check.validInput(newBookCount, 0, MAX_BOOKS):
                ...
                print("\nYou entered {0}, this will be updated in DB".format(
                    newBookCount))
